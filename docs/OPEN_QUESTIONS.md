...
There are currently no unresolved design questions for Phases 1-9. The 0.9.0-alpha.7 through 0.9.0-alpha.15 hardening work is deterministic-test complete, including live rebuild interoperability, fenced-response compatibility, completeness capture, responsive/flat operator UX, passive-lineage protection, virtual hidden-message rebuild, and the Alpha.15 integrity pass. Live SillyTavern/provider/browser acceptance, including timeout/cancel rebuild behavior, user-send timing, real Ternia registry import, Megumin display continuity, and explicit World_State coordinate extraction quality, is tracked via `docs/LIVE_ACCEPTANCE.md`.
