...
Desktop uses a centered workspace up to roughly 1180 px. Reality record views use flat inline disclosure rows so anchors/timeline/evidence/relations expand in place instead of a nested master/detail card; Places retains its adaptive list/detail editor. Tablet preserves the same disclosure model while Places adapts between split and single-pane detail. Mobile (<600 px) uses full-screen `100vw × 100dvh`, bottom navigation (Current / Places / Ops / More), safe-area-aware controls, >=44 px touch targets, and a full-height rebuild sheet. Dense operation/detail grids collapse progressively rather than shrinking typography.
...
Phase 7 does not add a generic slash-command family. The settings/panel host wires export/import/reset/rebuild to the existing Phase 5 services, preserving their preview/confirm and atomicity contracts. From 0.9.0-alpha.17, expanded active Reality records also expose `Mark resolved` and `Mark superseded`; these emit host-owned record-action intents, require an operator note, validate an opaque row snapshot against current canonical state, and then call the same Phase 5 manual reducer path. Raw record IDs remain absent from the public UI model.
...
snapshot canonical state + exact current chat lineage
   |
optionally build an immutable virtual view that treats eligible hidden roleplay rows as user/assistant for rebuild only
   + never mutate live chat visibility, is_system, saves, DOM, or lineage
   + keep genuine system/tool/UI rows excluded
   |
