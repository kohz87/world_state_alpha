...
**Phases 1-9 are implemented as candidates. Version 0.9.0-alpha.7 hardens the alpha.6 continuity model after focused rebuild/branch/Spatial audit reproductions. Rebuild now accepts only successful reconstruction boundaries and fails atomically on timeout, cancellation, or structurally malformed rows; unresolved branch recovery injects no Reality or Spatial continuity; elapsed-time catch-up uses sanitized established chronology evidence; normal capture sees a bounded resolved-tombstone admission slice; rebuild shares automatic Spatial authority protections; disabled Spatial state survives Reality-only rebuild; direct relations require field grounding; explicit current-location `World_State` headers can deterministically recover grounded coordinates; and provider-backed catch-up no longer holds `MESSAGE_SENT` open. Canonical schema remains version 2; sidecar, bundle, and rollback-journal envelope formats remain version 1.**
