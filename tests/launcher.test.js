...
function fakeDom() {
  ...
  return { doc, win, element, listeners, body };
}
...
test('launcher opens the panel on click, suppresses the click after a drag, and saves the dragged position', () => {
  ...
  listeners.get('click')({ preventDefault() {} });
  assert.equal(opened, 1, 'the click that ends a drag does not open the panel');
  assert.deepEqual(readLauncherPosition(storage), { left: 218, top: 400 });
  assert.equal(element.dataset.positioned, 'true');

  assert.equal(mountWorldStateLauncher({ onOpen() {}, doc, win, storage }), controller, 'mounting twice reuses the same button');
  controller.destroy();
...
test('launcher is World-State-owned, needs no other extension, and does not observe the DOM', () => {
  ...
  assert.doesNotMatch(source, /MutationObserver|setInterval|npc_state|npc-state|NPCState|Ukiyo|Megumin/);
